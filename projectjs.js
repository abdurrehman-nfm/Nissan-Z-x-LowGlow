...
window.onload=function(){
    document.getElementById('myForm').addEventListener("submit", function(event){
        ...
        let namePat = /^[A-Za-z]{0,100}$/;
        let nameError=document.getElementById("err1");

        let email = document.getElementById('emailinp').value.trim();
        let emailPat = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        let emailError=document.getElementById("err2");

        let contact = document.getElementById('continp').value.trim();
        let contactPat = /^[0-9]{0,11}$/;
        let contactError=document.getElementById("err3");

        let query = document.getElementById('nameinp').value.trim();
        let queryError=document.getElementById("err4");

        if(name==="" || name.search(namePat)==-1){
            nameError.innerHTML = "  Error";
				nameError.style.color = "red";
			}
		else{
				nameError.innerHTML = "valid data";
        ...
        if(email==="" || email.search(emailPat)==-1){
            emailError.innerHTML = "  Error";
				...
				emailError.innerHTML = "valid data";
				emailError.style.color = "green";
			}

        if(contact==="" || contact.search(contactPat)==-1){
            contactError.innerHTML = "  Error";
				contactError.style.color = "red";
			}
		else{
				contactError.innerHTML = "valid data";
				contactError.style.color = "green";
			}

        if(query==="" ){
            queryErrorError.innerHTML = "  Error";
				queryErrorError.style.color = "red";
			}
		else{
				queryError.innerHTML = "valid data";
				queryError.style.color = "green";
			}
